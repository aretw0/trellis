...
- **Decoupled Core:** The engine's core logic (`pkg/`) is pure. Adapters in `pkg/adapters/` connect it to external systems.
- **Library-First:** Trellis is designed to be embedded. All standard adapters are public and reusable.
- **CLI as Reference Implementation:** The `trellis` CLI is built entirely using the public API found in `pkg/`.
